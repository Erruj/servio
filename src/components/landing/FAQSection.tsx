...
export function FAQSection() {
  ...
  const items = t("marketing.faq.items", { returnObjects: true }) as Array<{
    q: string;
    a: string;
  }>;
  ...
  return (
    <section
      ...
      aria-label="Veelgestelde vragen over Servio"
    >
          ...
          <Accordion type="single" collapsible className="space-y-2">
            {items.map((faq, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className="bg-card rounded-xl px-5 data-[state=open]:border-border transition-all duration-200 animate-fade-in-up hover:border-border"
