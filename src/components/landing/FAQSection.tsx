      ...
      <div className="container mx-auto px-6">
        <Reveal className="max-w-xl mx-auto text-center mb-16">
          ...
          <p className="text-muted-foreground">
            ...
            <Link to={`${prefix}/contact`} className="text-primary hover:underline transition-colors duration-150">{t('marketing.faq.contactLink')}</Link>{' '}
            ...
            <StaggerGroup stagger={0.06} className="space-y-2">
              {items.map((faq, index) => (
                <StaggerItem key={index} y={12}>
                  <AccordionItem
                    value={`item-${index}`}
                    className="bg-card border border-border/40 rounded-xl px-5 data-[state=open]:border-border hover:border-border transition-all duration-200 ease-out"
                  >
