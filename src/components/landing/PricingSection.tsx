...
export function PricingSection() {
  ...
  const plans = (
    t("marketing.pricing.plans", { returnObjects: true }) as Array<{
      name: string;
      price: string;
      description: string;
      features: string[];
          ...
          {plans.map((plan, index) => (
            <div
              ...
              className={`relative rounded-xl p-6 transition-all duration-300 animate-fade-in-up group ${
                plan.popular
                  ? "bg-foreground text-background border-2 border-foreground scale-[1.02]"
            ...
            >
              ...
              <div className="mb-5">
                <h3
                  className={`text-lg font-medium mb-0.5 ${plan.popular ? "text-background" : "text-foreground"}`}
                >
                  {plan.name}
                </h3>
                <p
              ...
              <div className="mb-5">
                <span
                  className={`text-3xl font-semibold ${plan.popular ? "text-background" : "text-foreground"}`}
                >
                  €{plan.price}
                </span>
                <span
              ...
              <div
                className={`mb-5 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium ${
                  plan.popular
              ...
              <ul className="space-y-2.5 mb-6">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-center gap-2.5 text-sm">
                    <Check
                      className={`w-3.5 h-3.5 flex-shrink-0 ${plan.popular ? "text-background/70" : "text-muted-foreground"}`}
                    />
                    <span
                      className={
                        plan.popular
