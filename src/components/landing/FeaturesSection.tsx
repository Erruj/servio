...
export function FeaturesSection() {
  ...
  const items = t('marketing.features.items', { returnObjects: true }) as Array<{ title: string; description: string; result: string }>;
    ...
    <section id="features" className="relative py-24 md:py-32 bg-muted/30 overflow-hidden" aria-label="Servio features en voordelen">
      ...
      <div className="container mx-auto px-6 relative">
        <div className="max-w-2xl mx-auto text-center mb-16 animate-fade-in-up">
          <span className="text-sm font-medium text-primary mb-4 block">{t('marketing.features.eyebrow')}</span>
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
          {items.map((feature, index) => {
            ...
            return (
              ...
              >
                <div className={`absolute inset-0 bg-gradient-to-br ${s.tint} opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none`} />
                <div className="relative flex items-start gap-5">
                  <div className={`flex-shrink-0 w-12 h-12 rounded-xl ${s.iconBg} flex items-center justify-center group-hover:scale-110 transition-all duration-300`}>
                    <Icon className={`w-6 h-6 ${s.iconColor}`} strokeWidth={1.5} />
                  </div>
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-foreground mb-2">{feature.title}</h3>
                    <p className="text-muted-foreground leading-relaxed mb-4">{feature.description}</p>
                    <div className={`inline-flex items-center gap-2 text-sm font-medium ${s.iconColor} group-hover:gap-3 transition-all duration-300`}>
