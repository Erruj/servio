...
serve(async (req) => {
  try {
    ...
    const limitCheck = await checkMailboxLimit(supabase, userId, "gmail", emailAddress);
    if (!limitCheck.allowed) {
      console.log(`[gmail-callback] Mailbox limit reached for ${userId} (tier=${limitCheck.tier}, limit=${limitCheck.limit})`);
